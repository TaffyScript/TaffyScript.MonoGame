...
namespace TaffyScript.MonoGame
{
    ...
    public class ContentLoader
    {
        ...
        [WeakMethod]
        public static TsObject unload_folder(ITsInstance inst, TsObject[] args)
        {
        ...
        [WeakMethod]
        public static TsObject load_texture(ITsInstance inst, TsObject[] args)
        {
            return new TsTexture(args.Length == 2 ? Load<Texture2D>((string)args[0], (string)args[1]) : Load<Texture2D>((string)args[0]));
        }

        [WeakMethod]
        public static TsObject load_bmp_font_strip(ITsInstance inst, TsObject[] args)
        {
        ...
        [WeakMethod]
        public static TsObject load_sprite_font(ITsInstance inst, TsObject[] args)
        {
            return new TsSpriteFont(args.Length == 2 ? Load<SpriteFont>((string)args[0], (string)args[1]) : Load<SpriteFont>((string)args[0]));
        }

        [WeakMethod]
        public static TsObject load_song(ITsInstance inst, TsObject[] args)
        {
            return new TsSong(Load<Song>((string)args[0]));
        }

        [WeakMethod]
        public static TsObject load_sound(ITsInstance inst, TsObject[] args)
        {
